var app = angular.module('Main', ['ngRoute']);
	...
	var switchModal = function(id, d) { 
		...
		if(d === 'none'){
			m.className = m.className.replace(" in", "");m.style.display = "none";
		} else {
			m.className +=" in";m.style.display = d;
		}
	}, 
	...
	app.controller('crudControler', function($scope, $http, $location) {
		...
		$scope.retrieve = function(o) {
			...
			url += "&page=" + $scope.page;
			$scope.ajax(url, params, function(data) { callback(data) });
		};
		/* delete function */
		$scope.delete = function (obj) { 
			var id = $scope.ajax("a=delete&id=" + obj._id.$id, {}, function(data) {	$scope.form = [];$scope.notify("Record deleted!");$scope.retrieve(); });
		};
		/* create function */
		$scope.create = function () { 
			var id = $scope.ajax("a=insert", $scope.form, function(data) { $scope.retrieve();switchModal("modalNewUser", 'none');$scope.notify("New record!"); });
		};
		/* update form */
		$scope.update = function () { 
			var id = $scope.ajax("a=update&id=" + $scope._id, $scope.form, function(data) { $scope.retrieve();switchModal("modalNewUser", 'none');$scope.notify("Updated!"); });
		};
		...
		$scope.dispatch = function(event) {
					...
					$scope.retrieve({params:{_id:e.getAttribute('data-id')},callback:$scope.populateform});
		};
		/* retrieve data if pagination is clicked */
		$scope.switchPage = function(i) {$scope.page = i;$scope.retrieve();};
		/* submit function form - switch between update and create form */
		 $scope.submit = function(event) {
		 	var form = $scope.formCall;
			$scope.form['name'] = $scope.name ? this.name : ''; $scope.name = ''; 
			$scope.form['email'] = $scope.email ? this.email : ''; $scope.email = ''; 
			$scope.form['address'] = $scope.address ? this.address : ''; $scope.address = ''; 
			if(form =='update') {
				$scope.update();$scope._id = "";$scope.formCall = "create";	
			} else {
				$scope.create();
			}	
		 };
